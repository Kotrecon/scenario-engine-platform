...
- Root level и overrides менять на лету через `PUT /api/v1/logging/level`.
...
- Metadata API (`/api/metadata`) — логируется как обычный запрос.
...
- Metadata API (`/api/metadata`) — обрабатывается, Correlation ID добавляется.
...
- `OpenTelemetryOptions` в `appsettings.json`
- Регистрация через `ObservabilityExtensions.AddCustomOpenTelemetry()`
- Валидация при старте через `ValidateOnStart()`
...
- [`api.md`](./api.md) — API-эндпоинты
- [`TODO.md`](./TODO.md) — незавершённые задачи
...
| Элемент                  | Изменение                                         |
| ------------------------ | ------------------------------------------------- |
| Версия документа         | 1.0.0 → 1.1.0                                     |
| Контроллеры              | Роуты обновлены на `/api/v1/...` (API Versioning) |
| Request/Response Logging | Добавлено примечание про `/api/metadata`          |
| Correlation ID           | Добавлено примечание про `/api/metadata`          |
| OpenTelemetry            | Добавлена информация о конфигурации через Options |
