...
- ✅ `/dev/token` недоступен в Production — проверяется через TestWebApplicationFactory с `ASPNETCORE_ENVIRONMENT=Production`, ожидается 404
...
> Таблица считает описанные сценарии (чекбоксы), а не `[Test]` методы.
> Реальное количество тестов: **202** (из `dotnet test`).

| Категория            | Всего   | ✅ Сделано | ⬜ Осталось |
| -------------------- | ------- | ---------- | ----------- |
| Юнит-тесты           | 143     | 143        | 0           |
| Интеграционные тесты | 23      | 19         | 4           |
| **Всего**            | **166** | **162**    | **4**       |

**Покрытие:** 55.5% строк (682/1229), 42.5% ветвлений (175/412)
**Тестов:** 202 (все проходят)
